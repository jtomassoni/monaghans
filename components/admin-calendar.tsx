...
interface CalendarViewProps {
  ...
  /** Rendered at the start of the toolbar row (e.g. Help). */
  toolbarLeft?: ReactNode;
  /** Rendered at the end of the toolbar row (e.g. Calendar/List toggle). */
  toolbarRight?: ReactNode;
}

type ViewMode = 'month' | 'week' | 'day';

export default function CalendarView({ events, specials, announcements = [], businessHours, calendarHours, onEventClick, onSpecialClick, onAnnouncementClick, onNewEvent, onNewAnnouncement, onEventUpdate, onEventAdded, onEventDeleted, toolbarLeft, toolbarRight }: CalendarViewProps) {
  ...
  return (
    <div className="flex flex-col h-full min-h-0" ref={calendarRef}>
        {/* Second bar: date nav + Day/Week/Month - horizontal scroll on narrow so all controls visible */}
      <div className="flex-shrink-0 min-w-0 border-b border-gray-300 dark:border-gray-700 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm shadow-sm py-1 sm:py-2 min-h-[36px] sm:min-h-0 overflow-x-auto">
        <div className="flex flex-row items-center gap-1 sm:gap-1.5 flex-nowrap min-w-max md:min-w-0 justify-start px-2 sm:px-4">
          {toolbarLeft && <div className="flex-shrink-0">{toolbarLeft}</div>}
          <button
            onClick={() => navigateDate('prev')}
            className="w-7 h-7 sm:w-9 sm:h-9 flex-shrink-0 flex items-center justify-center bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 cursor-pointer active:scale-95 touch-manipulation"
            aria-label="Previous"
          >
            <HiChevronLeft className="w-3.5 h-3.5 sm:w-5 sm:h-5 pointer-events-none" />
          </button>
          <div className="relative flex-shrink-0 min-w-[3.5rem] sm:min-w-[8rem] flex items-center justify-center px-0.5 sm:px-2">
              {viewMode === 'month' ? (
                <div className="flex items-center gap-0.5 sm:gap-2">
                  <button
                    ...
                    className="text-xs sm:text-xl font-bold text-gray-900 dark:text-white hover:text-blue-600 dark:hover:text-blue-400 transition-colors cursor-pointer active:scale-95 z-10 relative px-0.5 py-0 flex items-center touch-manipulation"
                  >
                ...
                <h2 className="text-xs sm:text-xl font-bold text-gray-900 dark:text-white text-center whitespace-nowrap px-0.5 sm:px-2 truncate max-w-[8rem] sm:max-w-none">
                  {`${format(weekStart, 'MMM d')}-${format(weekEnd, 'MMM d')}`}
                </h2>
              ) : viewMode === 'day' ? (
                <h2 className="text-xs sm:text-xl font-bold text-gray-900 dark:text-white text-center whitespace-nowrap px-0.5 sm:px-2">
                  <span className="hidden sm:inline">{format(currentDate, 'EEEE, MMM d, yyyy')}</span>
                  <span className="sm:hidden">{format(currentDate, 'EEE M/d')}</span>
                </h2>
              ) : (
                <button
                  data-year-trigger
                  ...
                  className="text-base sm:text-2xl font-bold text-gray-900 dark:text-white hover:text-blue-600 dark:hover:text-blue-400 transition-colors cursor-pointer active:scale-95 z-10 relative px-1 py-0.5 min-h-[32px] sm:min-h-0 flex items-center touch-manipulation"
                >
                  {format(currentDate, 'yyyy')}
                </button>
              )}
            ...
            <button
              ...
              aria-label="Next"
            >
              <HiChevronRight className="w-3.5 h-3.5 sm:w-5 sm:h-5 pointer-events-none" />
            </button>
            <button
              onClick={() => setCurrentDate(todayMT)}
              disabled={isViewingCurrentPeriod}
              className={`px-1.5 sm:px-4 py-1 sm:py-1.5 text-[10px] sm:text-xs rounded-md transition-all duration-200 font-semibold flex-shrink-0 border touch-manipulation ${
          ...
          {/* View Mode Switcher and Hours Config - same row as date nav */}
          <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
            ...
            <div className="flex gap-0.5 bg-gray-100 dark:bg-gray-800 rounded-md p-0.5 border border-gray-200 dark:border-gray-700 flex-shrink-0">
              <button
                onClick={() => setViewMode('day')}
                className={`px-1.5 sm:px-4 py-1 sm:py-2 rounded transition-all duration-200 flex items-center justify-center gap-0.5 sm:gap-1.5 text-[10px] sm:text-xs font-semibold cursor-pointer active:scale-95 touch-manipulation ${
                  viewMode === 'day'
              ...
              <button
                ...
                className={`hidden sm:flex px-3 sm:px-4 py-2 rounded transition-all duration-200 items-center justify-center gap-1.5 text-xs font-medium cursor-pointer active:scale-95 touch-manipulation ${
                  viewMode === 'month'
                    ? 'bg-white dark:bg-gray-700 text-blue-600 dark:text-blue-400 shadow-sm'
                    : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-white/50 dark:hover:bg-gray-700/50'
                }`}
