  ...
  const handleDuplicate = async (item: DailySpecial) => {
    try {
      ...
      if (res.ok) {
        const specialData = await res.json();
        const todayStr = getMountainTimeDateString(getMountainTimeToday());
        const isRecurring = getAppliesOnDays(specialData.appliesOn).length > 0;
        ...
        setEditingSpecial({
          ...
          priceNotes: specialData.priceNotes ?? null,
          type: specialData.type,
          appliesOn: isRecurring ? specialData.appliesOn : null,
          timeWindow: specialData.timeWindow ?? null,
          startDate: isRecurring ? null : todayStr,
          endDate: isRecurring ? null : todayStr,
          image: specialData.image ?? null,
    ...
    <div className="flex flex-col h-full overflow-hidden min-h-0">
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mb-3">
                      {item.appliesOn && getAppliesOnDays(item.appliesOn).length > 0 ? (
                        <span className="px-2.5 py-1 text-xs rounded-full font-medium flex-shrink-0 bg-orange-100 dark:bg-orange-900/40 text-orange-700 dark:text-orange-400 border border-orange-200 dark:border-orange-800 flex items-center gap-1.5">
      ...
      <SpecialModalForm
        ...
        special={editingSpecial ? {
          ...
          appliesOn: getAppliesOnDays(editingSpecial.appliesOn),
